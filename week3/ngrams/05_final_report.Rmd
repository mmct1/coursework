...
Because we are writing about things more in general, the trend in B actually shows the line in terms of counts going up
So while you could say that the years aren't getting mentioned as much in terms of comparison to the rest of the corpus,
the sheer volume of things (that has continued to increase) would be a reason for that. But the actual count of mentions 
doesn't seem to be disappearing in the same way.
...
halflife_df <- left_join(proportion_counts,select(peak_df,term,year,proportion),by="term") %>%
...
There seems to be a lot more outliers in the plot I created compared to the one from the paper, and the outliers have
a lot larger values. The trend lines are also different from one another, but do show one similarity in first displaying
a quicker decline, and then a less rapid decline as years increase.
